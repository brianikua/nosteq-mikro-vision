import { useState, useEffect, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import { supabase } from "@/integrations/supabase/client";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { SidebarProvider, SidebarTrigger } from "@/components/ui/sidebar";
import { AppSidebar } from "@/components/dashboard/AppSidebar";
import { VersionFooter } from "@/components/dashboard/VersionFooter";
import { UptimeReportTab } from "@/components/dashboard/UptimeReportTab";
import { Activity, AlertTriangle, BarChart3, Plus, Search } from "lucide-react";
import { toast } from "sonner";
import { useAutoLogout } from "@/hooks/use-auto-logout";

const abuseTypes = ["Botnet", "PortScan", "WebScraping", "SMTPSpam", "BruteForce", "DDoS", "Malware"];

const NetworkHealth = () => {
  useAutoLogout();
  const navigate = useNavigate();
  const [user, setUser] = useState<any>(null);
  const [isAdminOrAbove, setIsAdminOrAbove] = useState(false);

  // Abuse state
  const [reports, setReports] = useState<any[]>([]);
  const [devices, setDevices] = useState<any[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [searchAbuse, setSearchAbuse] = useState("");
  const [loadingAbuse, setLoadingAbuse] = useState(true);
  const [form, setForm] = useState({
    device_id: "", source_ip: "", abuse_type: "SMTPSpam", provider: "",
    strike_number: "1", raw_email_text: "", action_taken: "",
  });

  // Report state
  const [reportData, setReportData] = useState<any>(null);

  useEffect(() => {
    const init = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) { navigate("/auth"); return; }
      setUser(session.user);
      const { data: roleData } = await supabase.from("user_roles").select("role").eq("user_id", session.user.id).single();
      if (roleData && (roleData.role === "admin" || roleData.role === "superadmin")) setIsAdminOrAbove(true);
      fetchAbuse();
      fetchReportData();
    };
    init();
  }, [navigate]);

  const fetchAbuse = async () => {
    setLoadingAbuse(true);
    const [reportsRes, devicesRes] = await Promise.all([
      supabase.from("abuse_reports").select("*, devices(name)").order("created_at", { ascending: false }),
      supabase.from("devices").select("id, name").order("name"),
    ]);
    setReports(reportsRes.data || []);
    setDevices(devicesRes.data || []);
    setLoadingAbuse(false);
  };

  const fetchReportData = async () => {
    const [devRes, ipRes, abuseRes, blRes] = await Promise.all([
      supabase.from("devices").select("id, name, is_up").limit(500),
      supabase.from("ip_assignments").select("id, last_status, blacklist_count, monitor_uptime"),
      supabase.from("abuse_reports").select("id, source_ip, strike_number"),
      supabase.from("ip_downtime_events").select("id"),
    ]);

    const ips = ipRes.data || [];
    const abuseReports = abuseRes.data || [];
    const monitored = ips.filter((ip) => ip.monitor_uptime);
    const upCount = monitored.filter((ip) => ip.last_status === "up").length;
    const uptimePct = monitored.length > 0 ? Math.round((upCount / monitored.length) * 100 * 10) / 10 : 100;
    const blacklistedCount = ips.filter((ip) => (ip.blacklist_count || 0) > 0).length;

    // Repeat offenders: source IPs appearing 2+ times
    const sourceIpCounts: Record<string, number> = {};
    abuseReports.forEach((r) => { if (r.source_ip) sourceIpCounts[r.source_ip] = (sourceIpCounts[r.source_ip] || 0) + 1; });
    const repeatOffenders = Object.values(sourceIpCounts).filter((c) => c >= 2).length;

    // Simple health score
    let healthScore = 100;
    healthScore -= blacklistedCount * 10;
    healthScore -= abuseReports.length * 5;
    healthScore -= repeatOffenders * 15;
    healthScore = Math.max(0, Math.min(100, healthScore));

    setReportData({
      uptimePct,
      downtimeEvents: (blRes.data || []).length,
      blacklistedCount,
      abuseCount: abuseReports.length,
      repeatOffenders,
      healthScore,
      deviceCount: (devRes.data || []).length,
    });
  };

  const handleAddAbuse = async () => {
    if (!form.device_id || !form.abuse_type) { toast.error("Device and abuse type required"); return; }
    const { error } = await supabase.from("abuse_reports").insert({
      device_id: form.device_id,
      source_ip: form.source_ip || null,
      abuse_type: form.abuse_type,
      provider: form.provider || null,
      strike_number: parseInt(form.strike_number) || 1,
      raw_email_text: form.raw_email_text || null,
      action_taken: form.action_taken || null,
    });
    if (error) { toast.error(error.message); return; }
    toast.success("Abuse report logged");
    setShowAdd(false);
    setForm({ device_id: "", source_ip: "", abuse_type: "SMTPSpam", provider: "", strike_number: "1", raw_email_text: "", action_taken: "" });
    fetchAbuse();
    fetchReportData();
  };

  const filteredAbuse = reports.filter((r) =>
    r.source_ip?.includes(searchAbuse) || r.abuse_type?.toLowerCase().includes(searchAbuse.toLowerCase()) || r.devices?.name?.toLowerCase().includes(searchAbuse.toLowerCase())
  );

  const statusColors: Record<string, string> = {
    new: "bg-primary/20 text-primary",
    investigating: "bg-warning/20 text-warning",
    notified: "bg-accent/20 text-accent",
    resolved: "bg-success/20 text-success",
    repeat: "bg-destructive/20 text-destructive",
  };

  const getHealthColor = (score: number) => score > 80 ? "text-success" : score > 50 ? "text-warning" : "text-destructive";
  const getHealthBg = (score: number) => score > 80 ? "bg-success" : score > 50 ? "bg-warning" : "bg-destructive";

  if (!user) return null;

  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full">
        <AppSidebar activeTab="network-health" onTabChange={() => {}} isAdminOrAbove={isAdminOrAbove} userEmail={user.email} />

        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-[60px] flex items-center justify-between px-4 border-b border-border/50 bg-background/95 backdrop-blur-xl sticky top-0 z-50">
            <div className="flex items-center gap-3">
              <SidebarTrigger className="text-muted-foreground hover:text-foreground" />
              <div>
                <h1 className="text-base font-semibold text-foreground">Network Health</h1>
                <p className="text-[11px] text-muted-foreground">Uptime analytics, abuse tracking & health reports</p>
              </div>
            </div>
          </header>

          <main className="flex-1 p-4 md:p-6 overflow-auto animate-in fade-in duration-200">
            <Tabs defaultValue="uptime" className="space-y-4">
              <TabsList className="glass">
                <TabsTrigger value="uptime" className="flex items-center gap-1.5">
                  <Activity className="h-3.5 w-3.5" /> Uptime
                </TabsTrigger>
                <TabsTrigger value="abuse" className="flex items-center gap-1.5">
                  <AlertTriangle className="h-3.5 w-3.5" /> Abuse
                </TabsTrigger>
                <TabsTrigger value="report" className="flex items-center gap-1.5">
                  <BarChart3 className="h-3.5 w-3.5" /> Report
                </TabsTrigger>
              </TabsList>

              <TabsContent value="uptime">
                <UptimeReportTab />
              </TabsContent>

              <TabsContent value="abuse">
                <div className="space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="relative max-w-sm flex-1">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                      <Input placeholder="Search reports..." className="pl-9 h-9" value={searchAbuse} onChange={(e) => setSearchAbuse(e.target.value)} />
                    </div>
                    <Button size="sm" className="h-8 text-xs gradient-primary text-primary-foreground" onClick={() => setShowAdd(true)}>
                      <Plus className="h-3.5 w-3.5 mr-1.5" /> Log Report
                    </Button>
                  </div>

                  {loadingAbuse ? (
                    <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-16 bg-card animate-pulse rounded-xl" />)}</div>
                  ) : filteredAbuse.length === 0 ? (
                    <div className="text-center py-12">
                      <AlertTriangle className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                      <h3 className="text-lg font-semibold text-foreground">No abuse reports</h3>
                      <p className="text-sm text-muted-foreground mt-1">Log abuse complaints when received from providers</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {filteredAbuse.map((r) => (
                        <div key={r.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-semibold text-sm text-foreground">{r.devices?.name || "Unknown"}</span>
                              <Badge variant="outline" className="text-[10px]">{r.abuse_type}</Badge>
                              <Badge className={`text-[10px] border-0 ${statusColors[r.status] || ""}`}>{r.status}</Badge>
                              {r.strike_number >= 2 && <Badge className="text-[10px] bg-destructive/20 text-destructive border-0">Repeat #{r.strike_number}</Badge>}
                            </div>
                            <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                              {r.source_ip && <span>Source: {r.source_ip}</span>}
                              {r.provider && <span>Provider: {r.provider}</span>}
                              <span>{new Date(r.created_at).toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                    </div>
                  )}
                </div>
              </TabsContent>

              <TabsContent value="report">
                {reportData && (
                  <div className="space-y-6">
                    {/* Health Score */}
                    <Card className="border-border/50">
                      <CardContent className="pt-6">
                        <div className="flex items-center gap-6">
                          <div className="relative h-24 w-24">
                            <svg className="h-24 w-24 -rotate-90" viewBox="0 0 100 100">
                              <circle cx="50" cy="50" r="40" fill="none" stroke="hsl(var(--border))" strokeWidth="8" />
                              <circle cx="50" cy="50" r="40" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray={`${reportData.healthScore * 2.51} 251`} className={getHealthColor(reportData.healthScore)} strokeLinecap="round" />
                            </svg>
                            <div className="absolute inset-0 flex items-center justify-center">
                              <span className={`text-xl font-bold ${getHealthColor(reportData.healthScore)}`}>{reportData.healthScore}</span>
                            </div>
                          </div>
                          <div>
                            <h3 className="text-lg font-bold text-foreground">Subnet Health Score</h3>
                            <p className="text-sm text-muted-foreground">Based on blacklists, abuse reports, and uptime</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>

                    {/* Summary Cards */}
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {[
                        { label: "Overall Uptime", value: `${reportData.uptimePct}%`, color: "text-success" },
                        { label: "Downtime Events", value: reportData.downtimeEvents, color: "text-destructive" },
                        { label: "Blacklisted IPs", value: reportData.blacklistedCount, color: "text-warning" },
                        { label: "Abuse Reports", value: reportData.abuseCount, color: "text-primary" },
                        { label: "Repeat Offenders", value: reportData.repeatOffenders, color: "text-destructive" },
                        { label: "Devices Monitored", value: reportData.deviceCount, color: "text-primary" },
                      ].map((s) => (
                        <Card key={s.label} className="border-border/50">
                          <CardContent className="pt-4 pb-3 text-center">
                            <p className={`text-2xl font-bold ${s.color}`}>{s.value}</p>
                            <p className="text-xs text-muted-foreground">{s.label}</p>
                          </CardContent>
                        </Card>
                      ))}
                    </div>

                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => toast.info("Report sending coming soon")}>Send Report to Telegram</Button>
                    </div>
                  </div>
                )}
              </TabsContent>
            </Tabs>
          </main>
          <VersionFooter />
        </div>
      </div>

      {/* Add Abuse Dialog */}
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent>
          <DialogHeader><DialogTitle>Log Abuse Report</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-muted-foreground">Device *</label>
              <Select value={form.device_id} onValueChange={(v) => setForm({ ...form, device_id: v })}>
                <SelectTrigger><SelectValue placeholder="Select device" /></SelectTrigger>
                <SelectContent>{devices.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-muted-foreground">Source IP</label>
                <Input value={form.source_ip} onChange={(e) => setForm({ ...form, source_ip: e.target.value })} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground">Abuse Type *</label>
                <Select value={form.abuse_type} onValueChange={(v) => setForm({ ...form, abuse_type: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{abuseTypes.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <label className="text-xs text-muted-foreground">Provider</label>
                <Input value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value })} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground">Strike #</label>
                <Input type="number" value={form.strike_number} onChange={(e) => setForm({ ...form, strike_number: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="text-xs text-muted-foreground">Raw Abuse Email</label>
              <Textarea value={form.raw_email_text} onChange={(e) => setForm({ ...form, raw_email_text: e.target.value })} placeholder="Paste the abuse email text..." />
            </div>
            <div>
              <label className="text-xs text-muted-foreground">Action Taken</label>
              <Input value={form.action_taken} onChange={(e) => setForm({ ...form, action_taken: e.target.value })} />
            </div>
            <Button className="w-full gradient-primary text-primary-foreground" onClick={handleAddAbuse}>Log Report</Button>
          </div>
        </DialogContent>
      </Dialog>
    </SidebarProvider>
  );
};

export default NetworkHealth;
