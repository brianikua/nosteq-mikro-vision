...
interface BlacklistResult {
  ...
  confidence: number;
  raw: unknown;
  check_type: string; // "dnsbl" | "api" | "web"
}
...
async function checkDNSBL(
  ...
  try {
    ...
    // Try to determine category from return codes
...
async function checkVirusTotal(ip: string, apiKey: string): Promise<BlacklistResult> {
  try {
    ...
    return {
      ...
      category,
      confidence,
      raw: { malicious, suspicious, harmless: stats.harmless, undetected: stats.undetected, reputation: data.data?.attributes?.reputation },
      check_type: "api",
    };
  } catch (e) {
...
async function checkIPQualityScore(ip: string, apiKey: string): Promise<BlacklistResult> {
  try {
    ...
    return {
      ...
      raw: { fraud_score: fraudScore, proxy: d.proxy, vpn: d.vpn, tor: d.tor, bot: d.bot_status, isp: d.ISP, country: d.country_code },
      check_type: "api",
    };
  } catch (e) {
...
// ── ip-api.com geo & proxy check (free, no key) ──
async function checkIPApi(ip: string): Promise<BlacklistResult> {
  try {
    ...
    return {
      provider: "IP-API (Proxy/Hosting)",
      listed,
      category: listed ? "other" : null,
      confidence: listed ? 60 : 0,
      raw: d,
      check_type: "web",
    };
  } catch (e) {
...
// ── Blocklist.de check (free, no key) ──
async function checkBlocklistDe(ip: string): Promise<BlacklistResult> {
  try {
    ...
    return {
      ...
      listed,
      category: listed ? "brute_force" : null,
      confidence: listed ? 75 : 0,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const device of devices) {
      ...
      // Weighted scoring: API sources weigh more than DNSBL
